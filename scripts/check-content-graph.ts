...
async function parseAllowedLists() {
  ...
  if (!nodeMatch || !relationshipMatch)
    throw new Error('Unable to read allowed graph lists from docs/content-model.md.');
  ...
  return {
    allowedNodeTypes: split(nodeMatch[1]),
    allowedRelationships: split(relationshipMatch[1]),
  };
}
...
function parseFrontmatter(source: string): Record<string, FrontmatterValue> {
  ...
  for (const rawLine of lines) {
    ...
    if (arrayItem && currentArrayKey) {
      const list = Array.isArray(data[currentArrayKey])
        ? (data[currentArrayKey] as Array<Record<string, unknown>>)
        : [];
      const itemText = arrayItem[1];
      const objectMatch = itemText.match(/^([^:]+):\s*(.+)$/);
      currentArrayItem = objectMatch
        ? { [objectMatch[1].trim()]: parseScalar(objectMatch[2]) }
        : { target: parseScalar(itemText) };
...
async function collectContentNodes() {
  ...
  for (const collection of collections.filter((entry) => entry.isDirectory())) {
    ...
    for (const fullPath of files.filter((filePath) => /\.(md|mdx)$/.test(filePath))) {
      ...
      const slug = relative(fullPath)
        .replace(/^src\/content\//, '')
        .replace(/\.(md|mdx)$/, '')
        .replace(/\/index$/, '');
      const type = String(
        frontmatter.type ?? frontmatter.graph_type ?? singularType(collection.name),
      );
      ...
      for (const field of [
        ...
        for (const item of items) {
          ...
          edges.push({
            source: id,
            target,
            relationship: String(itemRecord?.relationship ?? defaultRelationship(field)),
            field,
            filePath: relative(fullPath),
            draft: frontmatter.draft === true,
          });
  ...
  for (const objectText of getArrayItems('nodes')) {
    ...
    if (id && type && title) {
      nodes.push({
        ...
        title,
        draft: false,
        filePath: relative(SITE_GRAPH_ROUTE_PATH),
        source: 'site-graph route',
      });
    }
  }
...
async function main() {
  if (!VALID_ENVS.includes(deployEnv as DeployEnv)) {
    addDiagnostic({
      code: 'DRG300',
      severity: 'error',
      message: `Invalid PUBLIC_DEPLOY_ENV "${deployEnv}".`,
      filePath: '.env',
      field: 'PUBLIC_DEPLOY_ENV',
      fix: `Set PUBLIC_DEPLOY_ENV to one of: ${VALID_ENVS.join(', ')}.`,
    });
  }
  const env = VALID_ENVS.includes(deployEnv as DeployEnv)
  ...
  for (const node of nodes.values()) {
    if (!allowedNodeTypes.has(node.type)) {
      addDiagnostic({
        code: 'DRG004',
        severity: 'error',
        message: `Invalid graph node type "${node.type}" for "${node.id}".`,
        filePath: node.filePath,
        field: 'type',
        target: node.id,
        fix: `Use one of the allowed node types from docs/content-model.md: ${[...allowedNodeTypes].join(', ')}.`,
      });
    }
  }
  ...
  for (const edge of edges) {
    if (!allowedRelationships.has(edge.relationship)) {
      addDiagnostic({
        code: 'DRG003',
        severity: 'error',
        message: `Invalid graph relationship "${edge.relationship}".`,
        filePath: edge.filePath,
        field: edge.field,
        target: edge.target,
        fix: `Use one of the allowed relationships from docs/content-model.md: ${[...allowedRelationships].join(', ')}.`,
      });
    }
    ...
    if (!source || !target) {
      addDiagnostic({
        code: 'DRG001',
        severity: 'error',
        message: `Referenced graph slug/id does not exist.`,
        filePath: edge.filePath,
        field: edge.field,
        target: !source ? edge.source : edge.target,
        fix: 'Check for a typo, add the missing content file/node, or remove the reference.',
      });
      continue;
    }
    ...
    if (!source.draft && target.draft) {
      addDiagnostic({
        code: 'DRG002',
        severity: env === 'production' ? 'error' : 'warning',
        message: `Published content "${source.id}" references draft content "${target.id}".`,
        filePath: edge.filePath,
        field: edge.field,
        target: target.id,
        fix: 'Publish the target, remove the reference, or keep the relationship out of production.',
      });
    }
  }
  ...
  for (const node of nodes.values()) {
    if (!node.draft && !connectedPublished.has(node.id)) {
      addDiagnostic({
        code: 'DRG010',
        severity: 'warning',
        message: `Published graph node "${node.id}" is orphaned.`,
        filePath: node.filePath,
        field: 'id',
        target: node.id,
        fix: 'Add at least one valid relationship to or from this published node, or intentionally leave it orphaned for Version 1.',
      });
    }
  }
  ...
  for (const diagnostic of diagnostics)
    console[diagnostic.severity === 'error' ? 'error' : 'warn'](formatDiagnostic(diagnostic));
